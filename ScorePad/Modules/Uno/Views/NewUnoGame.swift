...
struct NewUnoGame: View {
    @Environment(\.modelContext) private var modelContext
    @Environment(\.dismiss) private var dismiss
    @Query(sort: \PersonProfile.name) private var roster: [PersonProfile]

    @State private var edition: UnoEdition = .classic
    @State private var scoring: UnoScoring = .winnerCollects
    @State private var targetScore = UnoGame.defaultTargetScore
    ...
    var onSave: ((UnoGame.ID) -> Void)?

    static let maxPlayers = 10

    init(onSave: ((UnoGame.ID) -> Void)? = nil) {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    Picker("Game", selection: $edition) {
                        ForEach(UnoEdition.allCases) { Text($0.name).tag($0) }
                    }
                ...
                Section {
                    ...
                    if slots.count < Self.maxPlayers {
                        Button {
                ...
                Section {
                    Picker("Scoring", selection: $scoring) {
                        ForEach(UnoScoring.allCases) { Text($0.name).tag($0) }
                    }
                    Stepper(value: $targetScore, in: 50...2000, step: 50) {
                        LabeledContent("Play to", value: targetScore.formatted(.number.grouping(.never)))
                    }
                } header: {
                    Text("Scoring")
                } footer: {
    ...
    private func save() {
        ...
        let game = UnoGame(players: PlayerRef.seats(for: profiles),
                           edition: edition,
                           scoring: scoring,
                           targetScore: targetScore,
                           startingDealerIndex: slots.firstIndex { $0.id == dealerID } ?? 0)
...
#Preview {
    NewUnoGame()
        .modelContainer(for: [UnoGame.self, PersonProfile.self], inMemory: true)
}
